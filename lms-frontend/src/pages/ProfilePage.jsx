...
function ProfileHeaderCard({ user, onUserChange }) {
  ...
  const avatarUrl = useMemo(() => previewImage || getAvatarUrl(user.imagen?.startsWith("/uploads/") ? user.imagen.split("/").pop() : user.imagen), [previewImage, user?.imagen]);
    ...
    <div className="relative bg-white rounded-lg shadow-lg p-6 flex-1 min-h-[300px] border">
      ...
      {!loading && stats && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-2">
          {cards.map((card, idx) => (
            <div key={idx} className="relative bg-gray-50 rounded-lg p-4 shadow-md flex flex-col justify-between h-full border">
      ...
      {!loading && !stats && <p className="text-sm text-gray-600 mt-2">Sin datos de estadísticas.</p>}
    </div>
  );
}
  ...
  useEffect(() => {
    const loadStats = async () => {
      try {
        ...
        const data = await safeFetch(`${API_BASE_URL}/stats/${user.id}`, { method: "GET", headers: authHeaders() });
