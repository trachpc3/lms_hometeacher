        ...
        <aside className={`fixed inset-y-0 left-0 bg-white z-50 w-72 transition-transform transform duration-300 ease-in-out ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 md:relative md:flex md:w-72
            ...
            <div className="p-6 flex flex-col justify-between h-full w-full">
                ...
                <nav className="space-y-4">
                    <SidebarItem to="/dashboard-profesor" icon={LayoutDashboard} label="Inicio" onClick={() => setIsSidebarOpen && setIsSidebarOpen(false)} />
                    <SidebarItem to="/dashboard-profesor/alumnos" icon={Users} label="Mis Alumnos" onClick={() => setIsSidebarOpen && setIsSidebarOpen(false)} />
                    <SidebarItem to="/dashboard-profesor/renovaciones" icon={RefreshCw} label="Renovaciones" onClick={() => setIsSidebarOpen && setIsSidebarOpen(false)} />
                    <SidebarItem to="/dashboard-profesor/actividades" icon={ClipboardList} label="Actividades Pendientes" onClick={() => setIsSidebarOpen && setIsSidebarOpen(false)} />
                    <SidebarItem to="/dashboard-profesor/material" icon={BookOpen} label="Material de Clase" onClick={() => setIsSidebarOpen && setIsSidebarOpen(false)} />
                    <SidebarItem to="/dashboard-profesor/reportes" icon={BarChart} label="Reportes" onClick={() => setIsSidebarOpen && setIsSidebarOpen(false)} />
                </nav>
